...
parser.add_argument('--model-prefix', type=str, default="en_ko_mdl",
...
train_corpus =('korean_parallel_corpora/korean-english-v1/korean-english-park.train.en',
               'korean_parallel_corpora/korean-english-v1/korean-english-park.train.ko') 

test_corpus =('korean_parallel_corpora/korean-english-v1/korean-english-park.test.en', 
              'korean_parallel_corpora/korean-english-v1/korean-english-park.test.ko')
...
class preprocessing:
    ...
    def prepare_embedding(self, en, ko, filenm="embedding_train.txt"):
        en_ko = [ list(filter(None.__ne__, list(chain.from_iterable(list(zip_longest(i, j[1:-1]))))))  
                    for i, j in zip(en, ko) ]
        with open(filenm, "wt", encoding='UTF8') as f:
            f.writelines([" ".join(i) + '\n' for i in en_ko])
        return(filenm)
...
def model_init(n_hidden,vocab_size, embed_dim, max_seq_length, embed_weights, ctx, end_idx, attention=True):
    ...
    model = english_korean_translator(n_hidden, vocab_size, embed_dim, max_seq_length, end_idx, attention=True)
...
def train(epochs, tr_data_iterator, model, loss, trainer, ctx, start_epoch=1, mdl_desc="e2k_model", decay=False):
    ### 학습 코드 
    tot_test_loss = []
    tot_train_loss = []
...
if opt.embedding == True:
    ...
    test_ko  = en_ko_pre.extract_stem(test_corpus[0] ,l='en')
    test_en  = en_ko_pre.extract_stem(test_corpus[1] ,l='ko')
    
    fn = en_ko_pre.prepare_embedding(train_en + test_en, train_ko + test_ko)
    en_ko_pre.train_embedding([fn,], "en_ko.mdl", 'en_ko.np', 'en_ko.dic', 
                              min_count=10, 
                              iter=50, 
                              size=50, 
                              workers=10, 

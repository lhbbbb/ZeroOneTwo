...
if opt.embedding == True:
    ...
    ko_en_pre = preprocessing()
    
    train_ko = ko_en_pre.extract_stem(train_corpus[0],l='ko')
    train_en = ko_en_pre.extract_stem(train_corpus[1],l='en')
    test_ko  = ko_en_pre.extract_stem(test_corpus[0] ,l='ko')
    test_en  = ko_en_pre.extract_stem(test_corpus[1] ,l='en')
    
    fn = ko_en_pre.prepare_embedding(train_ko + test_ko, train_en + test_en)
    ko_en_pre.train_embedding([fn,], "ko_en.mdl", 'ko_en.np', 'ko_en.dic', 
                              min_count=10, 
                              iter=50, 
                              size=50, 
                              workers=10, 
